...
async function pollLoop(fetcher: () => Promise<unknown>, { interval = 500, dedupe = true, label = 'stream' }: PollOptions = {}) {
  ...
  while (running) {
    try {
      ...
      if (!dedupe || hash !== lastHash) {
        lastHash = hash;
        const line = JSON.stringify({ ...(data as Record<string, unknown>), _ts: Date.now(), _stream: label });
        process.stdout.write(line + '\n');
      }
    } catch (err: unknown) {
      if (/CDP|ECONNREFUSED/i.test((err as Error).message)) {
        await sleep(2000);
        continue;
      }
      process.stderr.write(`[stream:${label}] error: ${(err as Error).message}\n`);
    }
...
export async function streamQuote({ interval }: { interval?: number } = {}) {
...
export async function streamBars({ interval }: { interval?: number } = {}) {
...
export async function streamValues({ interval }: { interval?: number } = {}) {
  return pollLoop(fetchValues, { interval: interval || 500, label: 'values' });
}

// ── Stream: pine lines ──

async function fetchLines(studyFilter?: string) {
...
export async function streamLines({ interval, filter }: { interval?: number; filter?: string } = {}) {
  return pollLoop(() => fetchLines(filter), { interval: interval || 1000, label: 'lines' });
}

// ── Stream: pine labels ──

async function fetchLabels(studyFilter?: string) {
...
export async function streamLabels({ interval, filter }: { interval?: number; filter?: string } = {}) {
  return pollLoop(() => fetchLabels(filter), { interval: interval || 1000, label: 'labels' });
}

// ── Stream: pine tables ──

async function fetchTables(studyFilter?: string) {
  const filterStr = studyFilter ? JSON.stringify(studyFilter) : 'null';
  return evaluate(`
    (function() {
      var filter = ${filterStr};
...
export async function streamTables({ interval, filter }: { interval?: number; filter?: string } = {}) {
...
export async function streamAllPanes({ interval }: { interval?: number } = {}) {
