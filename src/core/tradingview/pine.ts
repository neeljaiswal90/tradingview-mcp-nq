import { evaluate, evaluateAsync, getClient } from '../session/manager.js';
...
export function analyze({ source }: { source: string }): AnalyzeResult {
  ...
  const arrays = new Map<string, { name: string; size: number | null; line: number }>();
...
export async function check({ source }: { source: string }): Promise<CheckResult> {
  ...
  const result = await response.json() as {
    result?: {
      errors2?: Array<{ start?: { line: number; column: number }; end?: { line: number; column: number }; message: string }>;
      warnings2?: Array<{ start?: { line: number; column: number }; message: string }>;
    };
    error?: string;
  };
...
export async function getSource(): Promise<{ success: boolean; source: string; line_count: number; char_count: number }> {
...
export async function setSource({ source }: { source: string }): Promise<{ success: boolean; lines_set: number }> {
  const editorReady = await ensurePineEditorOpen();
...
export async function compile(): Promise<{ success: boolean; button_clicked: string; source: string }> {
  const editorReady = await ensurePineEditorOpen();
  if (!editorReady) throw new Error('Could not open Pine Editor.');
...
export async function getErrors(): Promise<{ success: boolean; has_errors: boolean; error_count: number; errors: unknown[] }> {
  const editorReady = await ensurePineEditorOpen();
  if (!editorReady) throw new Error('Could not open Pine Editor.');
...
export async function save(): Promise<{ success: boolean; action: string }> {
  const editorReady = await ensurePineEditorOpen();
  if (!editorReady) throw new Error('Could not open Pine Editor.');
...
export async function getConsole(): Promise<{ success: boolean; entries: unknown[]; entry_count: number }> {
  const editorReady = await ensurePineEditorOpen();
  if (!editorReady) throw new Error('Could not open Pine Editor.');
...
export async function smartCompile(): Promise<{
  success: boolean;
  button_clicked: string;
  has_errors: boolean;
  errors: unknown[];
  study_added: boolean | null;
}> {
  const editorReady = await ensurePineEditorOpen();
  if (!editorReady) throw new Error('Could not open Pine Editor.');
...
export async function newScript({ type }: { type: string }): Promise<{ success: boolean; type: string; action: string; template: string }> {
  const editorReady = await ensurePineEditorOpen();
  if (!editorReady) throw new Error('Could not open Pine Editor.');

  const typeMap: Record<string, string> = { indicator: 'indicator', strategy: 'strategy', library: 'library' };
  ...
  return { success: true, type, action: 'new_script_created', template: typeMap[type] || type };
}

export async function openScript({ name }: { name: string }): Promise<{
  success: boolean;
  name: string;
  script_id: string;
  lines: number;
  ...
  `) as { error?: string; name?: string; id?: string; lines?: number } | null;
...
export async function listScripts(): Promise<{
  ...
  `) as { scripts?: unknown[]; error?: string } | null;
