import { getClient, getTargetInfo, evaluate } from '../session/manager.js';
...
export async function healthCheck(): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    chart_symbol: (state?.symbol as string) || 'unknown',
    chart_resolution: (state?.resolution as string) || 'unknown',
    chart_type: (state?.chartType as number) ?? null,
    api_available: (state?.apiAvailable as boolean) ?? false,
  };
}
...
export async function discover(): Promise<Record<string, unknown>> {
  ...
  `) as Record<string, { available: boolean; [key: string]: unknown }>;
...
export async function uiState(): Promise<Record<string, unknown>> {
  const state = await evaluate(`
    (function() {
      var ui = {};
...
/**
 * Attempt to launch TradingView via IApplicationActivationManager COM API.
 * Works for MSIX / Microsoft Store installs where the exe lives inside the
 * protected WindowsApps directory and can't be invoked directly.
 */
function launchMsixApp(cdpPort: number, killFirst: boolean): { pid: number; aumid: string; package: string } | null {
  try {
    if (killFirst) {
      try { execSync('taskkill /F /IM TradingView.exe', { timeout: 5000 }); } catch { /* may not be running */ }
    }

    const ps1 = resolve(dirname(fileURLToPath(import.meta.url)), '../../../scripts/launch_tv_store.ps1');
    if (!existsSync(ps1)) return null;

    const raw = execSync(
      `powershell -ExecutionPolicy Bypass -File "${ps1}" -Port ${cdpPort}`,
      { timeout: 30000 },
    ).toString().trim();

    const result = JSON.parse(raw) as { success: boolean; pid: number; aumid: string; package: string };
    if (result.success && result.pid) return result;
  } catch { /* Store app not installed or COM call failed */ }
  return null;
}
...
export async function launch(opts: { port?: number; kill_existing?: boolean } = {}): Promise<Record<string, unknown>> {
  const cdpPort = opts.port || 9222;
  const killFirst = opts.kill_existing !== false;
  ...
  // Try launching MSIX/Store app via IApplicationActivationManager COM API
  if (!tvPath && platform === 'win32') {
    const storeResult = launchMsixApp(cdpPort, killFirst);
    if (storeResult) {
      return await waitForCdp(cdpPort, {
        success: true, platform, binary: 'MSIX:' + storeResult.aumid,
        pid: storeResult.pid, cdp_port: cdpPort,
        cdp_url: `http://localhost:${cdpPort}`,
        package: storeResult.package,
        launch_method: 'IApplicationActivationManager',
      });
    }
  }
