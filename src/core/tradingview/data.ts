...
interface RawGraphicsItem {
  id: unknown;
  raw: Record<string, unknown>;
}

interface RawGraphicsStudy {
  name: string;
  count: number;
...
export async function getOhlcv({ count, summary, paneIndex }: { count?: number; summary?: boolean; paneIndex?: number } = {}) {
  ...
  if (!data || !data.bars || (data.bars as unknown[]).length === 0) {
    throw new Error('Could not extract OHLCV data. The chart may still be loading.');
  }

  if (summary) {
    const bars = data.bars as Record<string, number>[];
    ...
    return {
      ...
      high: Math.max(...(highs as number[])), low: Math.min(...(lows as number[])),
      range: Math.round((Math.max(...(highs as number[])) - Math.min(...(lows as number[]))) * 100) / 100,
      change: Math.round((last.close! - first.open!) * 100) / 100,
      change_pct: Math.round(((last.close! - first.open!) / first.open!) * 10000) / 100 + '%',
      avg_volume: Math.round((volumes as number[]).reduce((a, b) => a + b, 0) / volumes.length),
...
export async function getIndicator({ entity_id, paneIndex }: { entity_id: string; paneIndex?: number }) {
  ...
  let inputs = data?.inputs as Record<string, unknown>[] | null;
...
export async function getTrades({ max_trades, paneIndex }: { max_trades?: number; paneIndex?: number } = {}) {
  ...
  `) as Record<string, unknown>;
  const tradeList = (trades?.trades ?? []) as unknown[];
...
export async function getEquity({ paneIndex }: { paneIndex?: number } = {}) {
  ...
  `) as Record<string, unknown>;
  const eqData = (equity?.data ?? []) as unknown[];
...
export async function getDepth() {
  ...
  if (!data || !data.found) throw new Error((data?.error as string) || 'DOM panel not found.');
...
export async function getPineLines({ study_filter, verbose, paneIndex }: { study_filter?: string; verbose?: boolean; paneIndex?: number } = {}) {
  ...
  const studies = raw.map(s => {
    ...
    for (const item of s.items) {
      const v = item.raw;
      const y1 = v.y1 != null ? Math.round((v.y1 as number) * 100) / 100 : null;
      const y2 = v.y2 != null ? Math.round((v.y2 as number) * 100) / 100 : null;
    ...
    const result: Record<string, unknown> = { name: s.name, total_lines: s.count, horizontal_levels: hLevels };
...
export async function getPineLabels({ study_filter, max_labels, verbose, paneIndex }: { study_filter?: string; max_labels?: number; verbose?: boolean; paneIndex?: number } = {}) {
  ...
  const studies = raw.map(s => {
    let labels = s.items.map(item => {
      const v = item.raw;
      const text = (v.t as string) || '';
      const price = v.y != null ? Math.round((v.y as number) * 100) / 100 : null;
...
export async function getPineTables({ study_filter, paneIndex }: { study_filter?: string; paneIndex?: number } = {}) {
  ...
  const studies = raw.map(s => {
    const tables: Record<number, Record<number, Record<number, string>>> = {};
    for (const item of s.items) {
      const v = item.raw;
      const tid = (v.tid as number) || 0;
      const row = v.row as number;
      const col = v.col as number;
      if (!tables[tid]) tables[tid] = {};
      if (!tables[tid][row]) tables[tid][row] = {};
      tables[tid][row][col] = (v.t as string) || '';
    }
...
export async function getPineBoxes({ study_filter, verbose, paneIndex }: { study_filter?: string; verbose?: boolean; paneIndex?: number } = {}) {
  ...
  const studies = raw.map(s => {
    const zones: { high: number; low: number }[] = [];
    const seen: Record<string, boolean> = {};
    const allBoxes: Record<string, unknown>[] = [];
    for (const item of s.items) {
      const v = item.raw;
      const high = v.y1 != null && v.y2 != null ? Math.round(Math.max(v.y1 as number, v.y2 as number) * 100) / 100 : null;
      const low = v.y1 != null && v.y2 != null ? Math.round(Math.min(v.y1 as number, v.y2 as number) * 100) / 100 : null;
    ...
    const result: Record<string, unknown> = { name: s.name, total_boxes: s.count, zones };
