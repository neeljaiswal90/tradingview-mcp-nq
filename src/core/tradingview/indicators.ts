...
export async function setInputs({ entity_id, inputs: inputsRaw }: { entity_id: string; inputs: string | Record<string, unknown> }) {
  ...
  `) as { error?: string; updated_inputs?: Record<string, unknown> };

  if (result && result.error) throw new Error(result.error);
  return { success: true, entity_id, updated_inputs: result.updated_inputs };
}

export async function toggleVisibility({ entity_id, visible }: { entity_id: string; visible: boolean }) {
  if (!entity_id) throw new Error('entity_id is required. Use chart_get_state to find study IDs.');
  ...
  `) as { error?: string; visible?: boolean };
