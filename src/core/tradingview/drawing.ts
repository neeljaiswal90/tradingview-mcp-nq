...
import { getChartApi as _getChartApi } from '../session/manager.js';
import { safeString, requireFinite } from '../cdp/evaluate.js';

interface Deps {
  evaluate?: typeof _evaluate;
  getChartApi?: typeof _getChartApi;
}
...
export async function drawShape({
  ...
  const before = await evaluate(`${apiPath}.getAllShapes().map(function(s) { return s.id; })`) as string[] | null;
  ...
  const after = await evaluate(`${apiPath}.getAllShapes().map(function(s) { return s.id; })`) as string[] | null;
...
export async function getProperties({ entity_id }: { entity_id: string }) {
  const apiPath = await _getChartApi();
...
export async function removeOne({ entity_id }: { entity_id: string }) {
  const apiPath = await _getChartApi();
  const result = await _evaluate(`
    (function() {
      var api = ${apiPath};
