...
export async function click({ by, value }: { by: string; value: string }) {
  ...
  `) as { found: boolean; tag?: string; text?: string; aria_label?: string | null; data_name?: string | null } | null;
  if (!result || !result.found) throw new Error('No matching element found for ' + by + '="' + value + '"');
  return { success: true, clicked: result };
}

export async function openPanel({ panel, action }: { panel: string; action: string }) {
  ...
  if (isBottomPanel) {
    ...
    `) as { error?: string; was_open?: boolean; performed?: string } | null;
    if (result && result.error) throw new Error(result.error);
    return { success: true, panel, action, was_open: result?.was_open ?? false, performed: result?.performed ?? 'unknown' };
  } else {
    const selectorMap: Record<string, { dataName: string; ariaLabel: string }> = {
...
export async function layoutList() {
  ...
  `) as { layouts?: unknown[]; source?: string; error?: string } | null;
...
export async function layoutSwitch({ name }: { name: string }) {
  ...
  `) as { success?: boolean; error?: string; name?: string; id?: string; source?: string } | null;
...
export async function keyboard({ key, modifiers }: { key: string; modifiers?: string[] }) {
...
export async function typeText({ text }: { text: string }) {
  const c = await getClient();
  await c.Input.insertText({ text });
  return { success: true, typed: text.substring(0, 100), length: text.length };
}

export async function hover({ by, value }: { by: string; value: string }) {
...
export async function scroll({ direction, amount }: { direction: string; amount?: number }) {
  const c = await getClient();
  const px = amount || 300;
...
export async function mouseClick({ x, y, button, double_click }: { x: number; y: number; button?: string; double_click?: boolean }) {
  const c = await getClient();
...
export async function findElement({ query, strategy }: { query: string; strategy?: string }) {
...
export async function uiEvaluate({ expression }: { expression: string }) {
