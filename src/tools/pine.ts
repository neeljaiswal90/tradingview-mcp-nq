...
import * as core from '../core/tradingview/pine.js';

export function registerPineTools(server: McpServer): void {
  ...
  server.tool('pine_open', 'Open a saved Pine Script by name', {
    ...
    catch (err) { return jsonResult({ success: false, source: 'internal_api', error: (err as Error).message }, true); }
  });
