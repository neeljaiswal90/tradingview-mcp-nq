...
import * as core from '../core/tradingview/watchlist.js';
import { getClient } from '../core/session/manager.js';
...
export function registerWatchlistTools(server: McpServer): void {
  ...
  server.tool('watchlist_add', 'Add a symbol to the TradingView watchlist', {
    ...
    try { return jsonResult(await core.add({ symbol })); }
      ...
      return jsonResult({ success: false, error: (err as Error).message }, true);
    }
  });
}
