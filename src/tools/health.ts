...
import * as core from '../core/tradingview/health.js';
...
export function registerHealthTools(server: McpServer): void {
  server.tool('tv_health_check', 'Check CDP connection to TradingView and return current chart state', {}, async () => {
    ...
    catch (err) { return jsonResult({ success: false, error: (err as Error).message, hint: 'TradingView is not running with CDP enabled. Use the tv_launch tool to start it automatically.' }, true); }
  });
