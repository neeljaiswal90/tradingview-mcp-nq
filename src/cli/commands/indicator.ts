...
import * as chartCore from '../../core/tradingview/chart.js';
import * as indCore from '../../core/tradingview/indicators.js';
import * as dataCore from '../../core/tradingview/data.js';
  ...
  subcommands: new Map([
    ['add', {
      ...
      handler: (opts: Record<string, unknown>, positionals: string[]) => {
        ...
        return chartCore.manageIndicator({ action: 'add', indicator: positionals.join(' '), inputs: opts.inputs as string | undefined });
      },
    }],
    ['remove', {
    ...
    ['toggle', {
      ...
      handler: (opts: Record<string, unknown>, positionals: string[]) => {
        ...
        const visible = opts.hidden ? false : (opts.visible !== undefined ? (opts.visible as boolean) : true);
    ...
    ['set', {
      ...
      handler: (opts: Record<string, unknown>, positionals: string[]) => {
        ...
        return indCore.setInputs({ entity_id: positionals[0], inputs: opts.inputs as string });
      },
    }],
    ['get', {
