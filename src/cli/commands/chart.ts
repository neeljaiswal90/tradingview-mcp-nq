...
import * as core from '../../core/tradingview/chart.js';
import * as healthCore from '../../core/tradingview/health.js';
...
register('symbol', {
  ...
  handler: async (_opts, positionals) => {
    const sym = positionals[0];
    if (sym) return core.setSymbol({ symbol: sym });
    const state = await core.getState() as Record<string, unknown>;
...
register('type', {
  ...
  handler: async (_opts, positionals) => {
    ...
    return { success: true, chart_type: typeNames[state.chartType as number] || state.chartType, type_num: state.chartType };
  },
});
