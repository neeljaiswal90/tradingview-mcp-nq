...
register('ui', {
  ...
  subcommands: new Map([
    ['click', {
      ...
      handler: (opts: Record<string, unknown>) => core.click({ by: (opts.by as string) || 'text', value: opts.value as string }),
    }],
    ['keyboard', {
    ...
    ['hover', {
      ...
      handler: (opts: Record<string, unknown>) => core.hover({ by: (opts.by as string) || 'text', value: opts.value as string }),
    }],
    ['scroll', {
    ...
    ['find', {
      ...
      handler: (opts: Record<string, unknown>, positionals: string[]) => {
        ...
        return core.findElement({ query: positionals.join(' '), strategy: opts.strategy as string | undefined });
      },
    }],
    ['eval', {
    ...
    ['mouse', {
      ...
      handler: (opts: Record<string, unknown>, positionals: string[]) => {
        ...
        return core.mouseClick({
          ...
          double_click: opts.double as boolean | undefined,
        });
      },
    }],
  ]),
